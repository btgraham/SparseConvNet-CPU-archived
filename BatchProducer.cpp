...
void BatchProducer::batchProducerThread(int nThread) {
  ...
  for (int c=nThread;c<nBatches;c+=NBATCHPRODUCERTHREADS) {
    ...
    batchPool[cc]->interfaces[0].featuresPresent.vec=range(dataset.nFeatures);
    for (int i=c*batchSize;i<std::min((c+1)*batchSize,(int)(dataset.pictures.size()));i++) {
      ...
      batchPool[cc]->labels.vec.push_back(pic->label);
      pic->codifyInputData
        (batchPool[cc]->interfaces[0].grids.back(),
         batchPool[cc]->interfaces[0].sub->features.vec,
    ...
    if (cnn.inputNormalizingConstants.size()>0) {
      std::vector<float> &features=batchPool[cc]->interfaces[0].sub->features.vec;
